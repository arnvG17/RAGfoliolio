...
const CardSwap: React.FC<CardSwapProps> = ({
  ...
  const prevIndex = useRef<number>(activeIndex ?? 0);
  ...
  // Handle activeIndex changes (driven externally by scrolling)
  useEffect(() => {
    if (activeIndex === undefined) return;
    if (refs.length < 2) return;

    const total = refs.length;
    const targetIdx = Math.max(0, Math.min(total - 1, activeIndex));
    const prevFrontIndex = prevIndex.current;
    
    if (targetIdx === prevFrontIndex) return;
    prevIndex.current = targetIdx;

    // Build the target order of card indexes where targetIdx is at slot 0 (front)
  ...
  // Handle automatic swapping if activeIndex is NOT provided
  useEffect(() => {
    if (activeIndex !== undefined) return;
    const total = refs.length;
    if (total < 2) return;
  ...
  }, [cardDistance, verticalDistance, delay, pauseOnHover, skewAmount, easing, activeIndex, refs, config.durDrop, config.durReturn, config.durMove, config.ease, config.promoteOverlap, config.returnDelay]);
